...
void I2C_read_multiple(unsigned char address, unsigned char * data, int length) {
    int i;
    //starts reading all of the data, all length * 2 bytes of data
    i2c_master_start();
    i2c_master_send(0b11010100); // write register
    i2c_master_send(address); // inputted first register
    i2c_master_restart();
    i2c_master_send(0b11010101); // read register
    for (i = 0; i <= (length - 2); i = i + 1) {
        data[i] = i2c_master_recv();
        i2c_master_ack(0); // continue to read
    }
    data[13] = (unsigned char) i2c_master_recv();
    i2c_master_ack(1); // stop reading data
    i2c_master_stop();
}

void data_combine(unsigned char * data, signed short * modified_data, int length) {
    int ii;
    signed short A;
    signed short B;
    for (ii = 0; ii <= (length/2)-1; ii = ii + 1) {
        A = data[2*ii];
        B = data[2*ii + 1];
        B = B << 8;
        modified_data[ii] = (A | B);
    }
}