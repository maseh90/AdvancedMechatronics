...
void initSPI() {
    ...
    // set SDO1 - B13 -> SDA 
    //
    RPB13Rbits.RPB13R = 0b0011; // from SPI explanation videos, sets as SDO1
    // set B15 -> RES (reset), initialize to High
    //
    TRISBbits.TRISB15 = 0; // sets as output
    LATBbits.LATB15 = 1; // initialize to high
    // set B12 -> DC (data control), initialize Low
    //
    TRISBbits.TRISB12 = 0; // sets as output
    LATBbits.LATB15 = 0; // sets to LO initially
    // setup SPI1
